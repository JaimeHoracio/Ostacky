    ...
    agents: [
      {
        ...
        description: "Orquestador con recuperación automática (nunca se congela), ruteo por nivel de impacto, controller MCP con SDK oficial, edición segura con fallback inline, y delegación en OpenSpec + Superpowers. v0.9.0: prune de skills obsoletas, lastHandoff, getAvailableTransitions, consecutiveFailures real, mem_session_summary automático al cierre.",
        version: "0.9.0",
    ...
    commands: [
      {
        ...
        description: "Instala el stack tecnológico del proyecto (CodeGraph, skills, OpenSpec, Engram, controller plugin). v0.9.0: controller plugin como alma hard-gate, Context7 removido del stack.",
        version: "0.9.0",
    ...
    mcpServers: [
      {
        ...
        description: "Máquina de estados persistida con @modelcontextprotocol/server SDK. v0.9.0: 22 tools (incluye set_handoff, get_handoff, clear_handoff, get_available_transitions funcional). Bugfixes: consecutiveFailures real, lastHandoff state, defaultChoice persistido. Robustez: degraded mode automático tras 3 fallos, persistence condicional para Nivel 0, prune de skills obsoletas.",
        version: "0.9.0",
        sha256: "16780aaaa2469c2391cd1aa12ca56a0f4b88abaa80fa310672822b93a3815c9f"
      },
      {
        name: "openspec",
...
function checkDoctorV2(projectRoot) {
  ...
  if (found) {
    try {
      ...
      if (isV2)
        lines.push(`✅ controller: plugin V2 (${found})`);
      else
        lines.push(`⚠️ controller: plugin legacy V1 en ${found} — no corre en V2, reinstalá con ostacky`);
    } catch {
  ...
  if (serverNames.length > 0)
    lines.push(`✅ mcp.servers: OK (${serverNames.length}: ${serverNames.join(", ")})`);
  ...
  try {
    ...
    for (const dir of pathEnv.split(delimiter)) {
      if (!dir)
        continue;
      try {
        if (existsSync8(join10(dir, name + suffix)))
          return join10(dir, name + suffix);
        if (suffix && existsSync8(join10(dir, name)))
...
async function ensureOpencodeInstalled() {
  if (isOpencodeInstalled()) {
    ...
    if (major !== null && major < 2) {
      ...
      ye(`El instalador V2 reemplaza el binario V1. Desinstalá V1 (package manager) e instalá V2, luego re-ejecutá.
Docs: https://opencode.ai/v2/docs/migrate-v1`, "Actualización requerida");
      process.exit(1);
    }
    return;
  }
...
// src/cli.ts
init_fs();
import { existsSync as existsSync9, statSync as statSync3, readFileSync as readFileSync7, readdirSync as readdirSync3 } from "node:fs";
...
async function runDoctorCommand() {
  ...
  for (const t of tools) {
    ...
    const pExe = p + ".exe";
    check(`tool ${t}: ${existsSync9(p) || existsSync9(pExe) ? "found" : "missing"}`, existsSync9(p) || existsSync9(pExe), true);
  }
  try {
  ...
  try {
    ...
    if (coreSync) {
      ...
      const ok = readFileSync7(coreMcp, "utf-8").includes("STATES") && readFileSync7(corePlugin, "utf-8").includes("STATES");
    ...
    if (existsSync9(auditPath)) {
      ...
      const lines = readFileSync7(auditPath, "utf-8").split(`
