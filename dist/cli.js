...
function getCommandInvocation(command, args, platform = process.platform) {
    const requiresCmd =
        platform === 'win32' && (command.toLowerCase().endsWith('.cmd') || command === 'npm' || command === 'npx');
        ...
        fetch(url, { signal: controller.signal, headers: { 'User-Agent': USER_AGENT } })
            .then((res) => {
                ...
                const writable = new WritableStream({
                    write(chunk) {
                        return new Promise((ok, failWrite) =>
...
async function downloadAndExtract(url, destDir, stripComponents = 1, timeoutMs = 180000) {
    ...
    try {
        ...
        for (let component = 0; component < stripComponents; component++) {
            ...
            if (entries.length !== 1) {
                throw new Error(
                    `No se puede remover ${stripComponents} componente(s) del archive: estructura inesperada.`
                );
            }
            const next = join(stagedDir, entries[0]);
            if (!statSync(next).isDirectory()) {
                throw new Error(
                    `No se puede remover ${stripComponents} componente(s) del archive: falta directorio raíz.`
                );
            }
            stagedDir = next;
        }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries && shouldRetryDownload(lastError)) {
                ...
                console.error(
                    `[download] Intento ${attempt + 1} falló: ${lastError.message}. Reintentando en ${delay}ms...`
                );
...
`);
        const block =
            [OSTACKY_GITIGNORE_HEADER, ...OSTACKY_GITIGNORE_PATTERNS].join(`
`) +
            `
`;
        const newContent =
...
`);
    ...
    if (headerIdx !== -1) {
        ...
        while (
            ...
            (OSTACKY_GITIGNORE_PATTERNS.some((p) => lines[insertIdx].trim() === p) || lines[insertIdx].trim() === '')
        ) {
            insertIdx++;
        }
...
var USER_AGENT = 'ostacky-installer',
    ...
    OSTACKY_GITIGNORE_HEADER = '# Ostacky';
var init_fs = __esm(() => {
    init_security();
    OSTACKY_GITIGNORE_PATTERNS = [
        '.opencode/tools/',
        ...
        agents: [
            {
                ...
                description:
                    'Orquestador con recuperación automática (nunca se congela), ruteo por nivel de impacto, controller MCP con SDK oficial, edición segura con fallback inline, y delegación en OpenSpec + Superpowers. v0.9.1: prune de skills obsoletas, lastHandoff, getAvailableTransitions, consecutiveFailures real, mem_session_summary automático al cierre.',
                version: '0.9.1',
                sha256: 'ecd046b5eb1d604f5e88bc7e5c7c2bda7d6fe63786fad085a038253202c59eeb',
            },
        ],
        commands: [
            {
                ...
                description:
                    'Instala el stack tecnológico del proyecto (CodeGraph, skills, OpenSpec, Engram, controller plugin). v0.9.1: controller plugin como alma hard-gate, Context7 removido del stack.',
                version: '0.9.1',
                sha256: '5cdceea5d8c4ef8eb2d1dcc441ad05f87c8c666dcf9810eb0766c9446840b57c',
            },
            {
                name: 'opsx-sync',
...
`,
        ...
        if (existsSync4(tmpPath)) rmSync2(tmpPath, { force: true });
    }
}
...
function isSuperpowersPluginEntry(entry) {
    try {
...
function copyOstackyControllerPlugin(projectRoot) {
    ...
    if (existsSync6(coreSource)) copyFileSync3(coreSource, join7(pluginsDir, 'controller-core.ts'));
    if (existsSync6(securitySource)) copyFileSync3(securitySource, join7(pluginsDir, 'security.ts'));
    if (existsSync6(tieredSource)) copyFileSync3(tieredSource, join7(pluginsDir, 'tiered.ts'));
}
...
async function installCodeGraph(toolsDir) {
    ...
    if (!existsSync6(cgToolDir)) mkdirSync4(cgToolDir, { recursive: true });
    ...
    if (!localBin) {
        ...
        if (!found) {
            return failAfterExtraction(
                `Descarga de CodeGraph ${tag} completada pero no se encontró el binario en ${join7(cgToolDir, 'bin')}.`
            );
        }
        try {
            ...
            return failAfterExtraction(
                `CodeGraph fue descargado pero no se pudo materializar el binario local: ${error.message}`
            );
        }
...
async function installEngram(toolsDir) {
    ...
    if (!existsSync6(engramBinDir)) mkdirSync4(engramBinDir, { recursive: true });
    ...
    if (!existsSync6(localBin)) {
        ...
        if (!found) {
            return failAfterExtraction(
                `Descarga de Engram ${tag} completada pero no se encontró el binario en ${localBin}.`
            );
        }
        try {
            if (found !== localBin) copyFileSync3(found, localBin);
        } catch (error) {
            return failAfterExtraction(
                `Engram fue descargado pero no se pudo materializar el binario local: ${error.message}`
            );
        }
...
import { join as join10, delimiter, isAbsolute } from 'path';
function checkDoctorV2(projectRoot) {
    const lines = [];
    ...
    const found = pluginPaths.find((p) => existsSync8(p));
    if (found) {
        try {
            const src = readFileSync6(found, 'utf-8');
            const isV2 =
                src.includes('Plugin.define') &&
                src.includes('@opencode/plugin') &&
                !src.includes('@opencode-ai/plugin');
    ...
    for (const [key, value] of Object.entries(mcp)) {
        ...
        if (value && typeof value === 'object' && ('command' in value || 'enabled' in value || 'url' in value)) {
            lines.push(
                `⚠️ mcp.${key} legacy (forma V1) — mové a mcp.servers.${key} con disabled (install lo migra con restore ante fallo)`
            );
        }
    }
    const renames = {
    ...
    for (const [oldKey, newKey] of Object.entries(renames)) {
        if (oldKey in config) {
            lines.push(
                `⚠️ ${oldKey} legacy ajeno — V2 usa ${newKey}; se normaliza en memoria, migrá cuando quieras (doctor no lo modifica)`
            );
        }
    }
    for (const [name, entry] of Object.entries(servers)) {
        ...
        if (entry.type === 'local' && Array.isArray(cmd) && typeof cmd[0] === 'string') {
            const bin = cmd[0];
            const ok = isAbsolute(bin) ? existsSync8(bin) : lookupOnPath(bin) !== null;
            if (!ok)
                lines.push(`⚠️ mcp.servers.${name} command no existe (${bin}) — ¿proyecto movido? re-corré install`);
        }
    }
    return lines;
}
function lookupOnPath(name) {
    try {
        ...
        for (const dir of pathEnv.split(delimiter)) {
            if (!dir) continue;
            try {
                if (existsSync8(join10(dir, name + suffix))) return join10(dir, name + suffix);
                if (suffix && existsSync8(join10(dir, name))) return join10(dir, name);
            } catch {}
        }
    } catch {}
    return null;
}
var init_doctor_v2 = __esm(() => {
    init_config();
    init_fs();
});
// package.json
var package_default = {
    ...
    files: ['dist/', 'assets/', 'manifest.json', 'LICENSE'],
    keywords: ['opencode', 'installer', 'cli', 'agent'],
    license: 'MIT',
    dependencies: {
        '@clack/prompts': '^0.9.1',
    },
...
(function (t) {
    var u = {};
    ((t.exports = u),
        (u.eastAsianWidth = function (e) {
            var s = e.charCodeAt(0),
                i = e.length == 2 ? e.charCodeAt(1) : 0,
                D = s;
            return (
                55296 <= s &&
                    s <= 56319 &&
...
function sD() {
    ...
    return (
        Object.defineProperty(r, 'codes', { value: t, enumerable: false }),
        ...
        (r.color.ansi16m = I()),
        (r.bgColor.ansi = L(m)),
        (r.bgColor.ansi256 = N(m)),
        (r.bgColor.ansi16m = I(m)),
        Object.defineProperties(r, {
            ...
            rgbToAnsi: { value: (u, F, e) => r.ansi256ToAnsi(r.rgbToAnsi256(u, F, e)), enumerable: false },
...
var _ = (t, u, F) => {
    ...
    for (const [C, o] of e.entries()) {
        const E = A(o);
        if (
            (D + E <= F ? (t[t.length - 1] += o) : (t.push(o), (D = 0)),
            v.has(o) &&
                ((s = true),
                (i = e
                    .slice(C + 1)
...
var oD = (t, u, F = {}) => {
    ...
    for (const [E, a] of t.split(' ').entries()) {
        ...
        if (
            (E !== 0 &&
                (n >= u && (F.wordWrap === false || F.trim === false) && (C.push(''), (n = 0)),
                (n > 0 || F.trim === false) && ((C[C.length - 1] += ' '), n++)),
            F.hard && D[E] > u)
        ) {
            const B = u - n,
                p = 1 + Math.floor((D[E] - B - 1) / u);
...
`),
    ...
    for (const [E, a] of o.entries()) {
        if (((e += a), v.has(a))) {
            const { groups: B } = new RegExp(`(?:\\${W}(?<code>\\d+)m|\\${y}(?<uri>.*)${w})`).exec(
                o.slice(E).join('')
            ) || { groups: {} };
            if (B.code !== undefined) {
                const p = Number.parseFloat(B.code);
                s = p === CD ? undefined : p;
            } else B.uri !== undefined && (i = B.uri.length === 0 ? undefined : B.uri);
        }
...
function hD(t) {
    for (const u in t) {
        ...
        switch (F) {
            case 'aliases': {
                for (const s in e) Object.hasOwn(e, s) && (c.aliases.has(s) || c.aliases.set(s, e[s]));
                break;
            }
        }
    }
...
function k(t, u) {
    ...
    for (const F of t) if (F !== undefined && k(F, u)) return true;
    return false;
}
function lD(t, u) {
    if (t === u) return;
...
function cD({ input: t = $, output: u = j, overwrite: F = true, hideCursor: e = true } = {}) {
    ...
    return (
        e && u.write(import_sisteransi.cursor.hide),
        t.once('keypress', i),
        () => {
            (t.off('keypress', i),
                e && u.write(import_sisteransi.cursor.show),
                t.isTTY && !xD && t.setRawMode(false),
                (s.terminal = false),
                s.close());
        }
    );
...
var pD = (t, u, F) =>
    u in t ? AD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
...
class x {
    ...
    prompt() {
        return new Promise((u, F) => {
            ...
            ((e._write = (s, i, D) => {
                ...
                M.emitKeypressEvents(this.input, this.rl),
                this.rl.prompt(),
    ...
    onKeypress(u, F) {
        if (
            ...
            F?.name &&
                (!this._track && c.aliases.has(F.name) && this.emit('cursor', c.aliases.get(F.name)),
                c.actions.has(F.name) && this.emit('cursor', F.name)),
        ...
        (k([u, F?.name, F?.sequence], 'cancel') && (this.state = 'cancel'),
...
`),
    ...
    restoreCursor() {
        const u =
            G(this._prevFrame, process.stdout.columns, { hard: true }).split(`
`).length - 1;
        ...
        if (u !== this._prevFrame) {
            if (this.state === 'initial') this.output.write(import_sisteransi.cursor.hide);
                ...
                if ((this.restoreCursor(), F && F?.length === 1)) {
                    const e = F[0];
                    (this.output.write(import_sisteransi.cursor.move(0, e)),
                        this.output.write(import_sisteransi.erase.lines(1)));
                    const s = u.split(`
`);
                    (this.output.write(s[e]),
                        (this._prevFrame = u),
                        this.output.write(import_sisteransi.cursor.move(0, s.length - e - 1)));
                    return;
                }
                if (F && F?.length > 1) {
                    const e = F[0];
...
var vD = (t, u, F) =>
    u in t ? gD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
...
var dD = class extends x {
    constructor(u) {
        ...
        ((this.options = Object.entries(F).flatMap(([e, s]) => [
            ...
            (this.value = [...(u.initialValues ?? [])]),
            (this.cursor = Math.max(
                this.options.findIndex(({ value: e }) => e === u.cursorAt),
                0
            )),
            this.on('cursor', (e) => {
                switch (e) {
    ...
    toggleValue() {
        ...
        if (u.group === true) {
            ...
            (this.isGroupSelected(F)
                ? (this.value = this.value.filter((s) => e.findIndex((i) => i.value === s) === -1))
                : (this.value = [...this.value, ...e.map((s) => s.value)]),
...
var mD = (t, u, F) =>
    u in t ? bD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
...
var wD = class extends x {
    constructor(u) {
        (super(u, false),
            ...
            (this.value = [...(u.initialValues ?? [])]),
            (this.cursor = Math.max(
                this.options.findIndex(({ value: F }) => F === u.cursorAt),
                0
            )),
            this.on('key', (F) => {
                F === 'a' && this.toggleAll();
...
var _D = (t, u, F) =>
    u in t ? yD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
var Z = (t, u, F) => (_D(t, typeof u != 'symbol' ? u + '' : u, F), F);

class kD extends x {
    constructor({ mask: u, ...F }) {
        (super(F),
            Z(this, 'valueWithCursor', ''),
            Z(this, '_mask', '•'),
            (this._mask = u ?? '•'),
            this.on('finalize', () => {
            ...
            this.on('value', () => {
                if (this.cursor >= this.value.length)
                    ...
                    const e = this.masked.slice(0, this.cursor),
                        s = this.masked.slice(this.cursor);
...
var $D = (t, u, F) =>
    u in t ? SD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
...
class jD extends x {
    constructor(u) {
        (super(u, false),
            ...
            q(this, 'cursor', 0),
            (this.options = u.options),
            (this.cursor = this.options.findIndex(({ value: F }) => F === u.initialValue)),
...
var TD = (t, u, F) =>
    u in t ? MD(t, u, { enumerable: true, configurable: true, writable: true, value: F }) : (t[u] = F);
...
class OD extends x {
    constructor(u) {
        ...
        ((this.cursor = Math.max(F.indexOf(u.initialValue), 0)),
            this.on('key', (e) => {
...
var k2 = (s) => {
    const { cursor: n, options: t, style: i } = s,
        r = s.maxItems ?? Number.POSITIVE_INFINITY,
        c = Math.max(process.stdout.rows - 4, 0),
        o = Math.min(c, Math.max(r, 5));
    let l = 0;
    n >= l + o - 3 ? (l = Math.max(Math.min(n - o + 3, t.length - o), 0)) : n < l + 2 && (l = Math.max(n - 2, 0));
    const $ = o < t.length && l > 0,
        d = o < t.length && l + o < t.length;
...
`,
                t = s.placeholder
                    ? import_picocolors2.default.inverse(s.placeholder[0]) +
                      import_picocolors2.default.dim(s.placeholder.slice(1))
                    : import_picocolors2.default.inverse(import_picocolors2.default.hidden('_')),
                i = this.value ? this.valueWithCursor : t;
            switch (this.state) {
                case 'error':
                    return `${n.trim()}
...
`;
                ...
                case 'cancel':
                    return `${n}${import_picocolors2.default.gray(a)}  ${import_picocolors2.default.strikethrough(import_picocolors2.default.dim(this.value ?? ''))}${
                        this.value?.trim()
                            ? `
...
`;
                ...
                case 'cancel':
                    return `${n}${import_picocolors2.default.gray(a)}  ${import_picocolors2.default.strikethrough(import_picocolors2.default.dim(i ?? ''))}${
                        i
                            ? `
${import_picocolors2.default.gray(a)}`
                            : ''
...
var me = (s) => {
    const n = s.active ?? 'Yes',
        t = s.inactive ?? 'No';
    return new fD({
        active: n,
        inactive: t,
        initialValue: s.initialValue ?? true,
        render() {
...
${import_picocolors2.default.gray(a)}`;
                default:
                    return `${t}${import_picocolors2.default.cyan(a)}  ${k2({
                        cursor: this.cursor,
...
var he = (s) => {
    const n = (t, i = 'inactive') => {
        const r = t.label ?? String(t.value);
        return i === 'selected'
            ? `${import_picocolors2.default.dim(r)}`
            : i === 'cancelled'
              ? `${import_picocolors2.default.strikethrough(import_picocolors2.default.dim(r))}`
              : i === 'active'
                ? `${import_picocolors2.default.bgCyan(import_picocolors2.default.gray(` ${t.value} `))} ${r} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`
                : `${import_picocolors2.default.gray(import_picocolors2.default.bgWhite(import_picocolors2.default.inverse(` ${t.value} `)))} ${r} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`;
    };
    return new OD({
        options: s.options,
        initialValue: s.initialValue,
...
${import_picocolors2.default.gray(a)}`;
                default:
                    return `${t}${import_picocolors2.default.cyan(a)}  ${this.options.map((i, r) =>
                        n(i, r === this.cursor ? 'active' : 'inactive')
                    ).join(`
${import_picocolors2.default.cyan(a)}  `)}
${import_picocolors2.default.cyan(m2)}
`;
...
var pe = (s) => {
    const n = (t, i) => {
        const r = t.label ?? String(t.value);
        return i === 'active'
            ? `${import_picocolors2.default.cyan(V2)} ${r} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`
            : i === 'selected'
              ? `${import_picocolors2.default.green(M2)} ${import_picocolors2.default.dim(r)}`
              : i === 'cancelled'
                ? `${import_picocolors2.default.strikethrough(import_picocolors2.default.dim(r))}`
                : i === 'active-selected'
                  ? `${import_picocolors2.default.green(M2)} ${r} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`
                  : i === 'submitted'
                    ? `${import_picocolors2.default.dim(r)}`
                    : `${import_picocolors2.default.dim(G2)} ${import_picocolors2.default.dim(r)}`;
    };
    return new wD({
        options: s.options,
                ...
                case 'submit':
                    return `${t}${import_picocolors2.default.gray(a)}  ${
                        this.options
                            ...
                            .map((r) => n(r, 'submitted'))
                            .join(import_picocolors2.default.dim(', ')) || import_picocolors2.default.dim('none')
                    }`;
                case 'cancel': {
...
`
                        ...
                        .map((c, o) =>
                            o === 0
                                ? `${import_picocolors2.default.yellow(m2)}  ${import_picocolors2.default.yellow(c)}`
                                : `   ${c}`
                        ).join(`
`);
                    return `${t + import_picocolors2.default.yellow(a)}  ${k2({
                        options: this.options,
                        cursor: this.cursor,
                        maxItems: s.maxItems,
                        style: i,
...
var ge = (s) => {
    const n = (t, i, r = []) => {
        const c = t.label ?? String(t.value),
            o = typeof t.group == 'string',
            l = o && (r[r.indexOf(t) + 1] ?? { group: true }),
            $ = o && l.group === true,
            d = o ? `${$ ? m2 : a} ` : '';
        return i === 'active'
            ? `${import_picocolors2.default.dim(d)}${import_picocolors2.default.cyan(V2)} ${c} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`
            : i === 'group-active'
              ? `${d}${import_picocolors2.default.cyan(V2)} ${import_picocolors2.default.dim(c)}`
              : i === 'group-active-selected'
                ? `${d}${import_picocolors2.default.green(M2)} ${import_picocolors2.default.dim(c)}`
                : i === 'selected'
                  ? `${import_picocolors2.default.dim(d)}${import_picocolors2.default.green(M2)} ${import_picocolors2.default.dim(c)}`
                  : i === 'cancelled'
                    ? `${import_picocolors2.default.strikethrough(import_picocolors2.default.dim(c))}`
                    : i === 'active-selected'
                      ? `${import_picocolors2.default.dim(d)}${import_picocolors2.default.green(M2)} ${c} ${t.hint ? import_picocolors2.default.dim(`(${t.hint})`) : ''}`
                      : i === 'submitted'
                        ? `${import_picocolors2.default.dim(c)}`
                        : `${import_picocolors2.default.dim(d)}${import_picocolors2.default.dim(G2)} ${import_picocolors2.default.dim(c)}`;
    };
    return new dD({
        options: s.options,
        initialValues: s.initialValues,
...
`;
            switch (this.state) {
                case 'submit':
                    return `${t}${import_picocolors2.default.gray(a)}  ${this.options
                        .filter(({ value: i }) => this.value.includes(i))
                        .map((i) => n(i, 'submitted'))
                        .join(import_picocolors2.default.dim(', '))}`;
                case 'cancel': {
...
`
                        ...
                        .map((r, c) =>
                            c === 0
                                ? `${import_picocolors2.default.yellow(m2)}  ${import_picocolors2.default.yellow(r)}`
                                : `   ${r}`
                        ).join(`
`);
                    return `${t}${import_picocolors2.default.yellow(a)}  ${this.options.map((r, c, o) => {
                        const l =
                            ...
                            $ = c === this.cursor;
                        return !$ && typeof r.group == 'string' && this.options[this.cursor].value === r.group
                            ? n(r, l ? 'group-active-selected' : 'group-active', o)
                            : $ && l
                              ? n(r, 'active-selected', o)
                              : l
                                ? n(r, 'selected', o)
                                : n(r, $ ? 'active' : 'inactive', o);
                    }).join(`
${import_picocolors2.default.yellow(a)}  `)}
${i}
`;
                ...
                default:
                    return `${t}${import_picocolors2.default.cyan(a)}  ${this.options.map((i, r, c) => {
                        const o =
                                ...
                                (i.group === true && this.isGroupSelected(`${i.value}`)),
                            l = r === this.cursor;
                        return !l && typeof i.group == 'string' && this.options[this.cursor].value === i.group
                            ? n(i, o ? 'group-active-selected' : 'group-active', c)
                            : l && o
                              ? n(i, 'active-selected', c)
                              : o
                                ? n(i, 'selected', c)
                                : n(i, l ? 'active' : 'inactive', c);
                    }).join(`
${import_picocolors2.default.cyan(a)}  `)}
${import_picocolors2.default.cyan(m2)}
`;
...
`),
        ...
        c = t.map(
            (o) =>
                `${import_picocolors2.default.gray(a)}  ${import_picocolors2.default.dim(o)}${' '.repeat(r - T2(o).length)}${import_picocolors2.default.gray(a)}`
        ).join(`
`);
...
var v2 = {
    message: (s = '', { symbol: n = import_picocolors2.default.gray(a) } = {}) => {
        const t = [`${import_picocolors2.default.gray(a)}`];
        if (s) {
            const [i, ...r] = s.split(`
`);
...
var L2 = () => {
    ...
    const $ = (h) => {
        ...
        w = () => $(1),
        b = () => {
            (process.on('uncaughtExceptionMonitor', d),
        ...
        C = () => {
            (process.removeListener('uncaughtExceptionMonitor', d),
...
`);
            const h = l.split(`
`);
            (process.stdout.write(import_sisteransi2.cursor.move(-999, h.length - 1)),
                process.stdout.write(import_sisteransi2.erase.down(h.length)));
        },
        x = (h) => h.replace(/\.+$/, ''),
        O = (h = '') => {
            ((c = true),
...
`));
            ...
            (b(),
                (r = setInterval(() => {
                    if (t && o === l) return;
                    (I(), (l = o));
                    const W = import_picocolors2.default.magenta(s[g]),
                        _ = t ? '...' : '.'.repeat(Math.floor(f)).slice(0, 3);
                    (process.stdout.write(`${W}  ${o}${_}`),
                        (g = g + 1 < s.length ? g + 1 : 0),
                        (f = f < s.length ? f + 0.125 : 0));
                }, n)));
        },
        P = (h = '', g = 0) => {
            ...
            const f =
                g === 0
                    ? import_picocolors2.default.green(S2)
                    : g === 1
                      ? import_picocolors2.default.red(A2)
                      : import_picocolors2.default.red(B);
            ((o = x(h ?? o)),
                process.stdout.write(`${f}  ${o}
`),
                C(),
                i());
        };
...
function validateMcpModule(nodeExecutable, serverPath, cwd) {
    try {
        execFileSync2(
            nodeExecutable,
            ['--input-type=module', '--eval', 'await import(process.env.OSTACKY_MCP_PROBE)'],
            {
                cwd,
...
function restoreFileSnapshot(snapshot) {
    if (snapshot.content === null) {
        if (existsSync5(snapshot.path)) rmSync3(snapshot.path, { force: true });
        return;
    }
...
function pruneStaleSkills(paths, manifest) {
    ...
    const removed = [];
    if (!existsSync5(paths.skills)) return removed;
...
`,
            ...
            try {
                ...
                throw new Error(
                    `No se pudieron instalar dependencias de ${item.name} con ${packageManager}: ${error.message}`
                );
            }
        }
        ...
        const probeOptions = isController
        ...
        await probeMcpServer(
            ...
            stagedServerPath,
            staging,
            isController ? join6(staging, '.probe-state.json') : undefined,
            probeOptions
        );
        ...
        try {
            setMcpEntryAtProjectRoot(
                projectRoot,
                item.name,
                createMcpConfigEntry(item.name, nodeExecutable, join6(dest, 'index.js'), statePath)
            );
        ...
        if (existsSync5(staging)) rmSync3(staging, { recursive: true, force: true });
    }
}
...
async function resolveOpenCodePaths(scope) {
    ...
    const dir =
        scope === 'local' || !scope ? getOpenCodeDirForScope('local', cwd) : getOpenCodeDirForScope('local', cwd);
...
async function doInstallStack(toolsDir, projectRoot) {
    ...
    const resolvedProjectRoot =
        projectRoot ?? dirname6(dirname6(toolsDir ?? join8(process.cwd(), '.opencode', 'tools')));
    ...
    try {
        ...
        if (inHome && !existsSync7(join8(home, '.git'))) {
            v2.warn(
                `Estás instalando el stack en tu home (${home}). Si esperabas instalar en un proyecto, hacé cd al proyecto y usá --scope local.`
            );
        }
    } catch {}
    ...
    try {
        ...
        if (gi.created) v2.info(`.gitignore creado con patrones Ostacky`);
        else if (gi.updated) v2.info(`.gitignore actualizado: ${gi.patternsAdded.join(', ')}`);
    } catch {}
    return allOk;
}
async function doInstallAll(manifest, paths) {
    ...
    if (!existsSync7(codegraphDir) || !findBinaryInDir(codegraphDir, 'codegraph')) missingTools.push('CodeGraph');
    if (!existsSync7(engramDir) || !findBinaryInDir(engramDir, 'engram')) missingTools.push('Engram');
...
function getOpencodeVersion() {
    try {
        return execFileSync4('opencode', ['--version'], {
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe'],
        }).trim();
    } catch {
...
async function ensureOpencodeInstalled() {
    if (isOpencodeInstalled()) {
        ...
        if (major !== null && major < 2) {
            v2.warn(`OpenCode V1 detectado (${raw}). Ostacky requiere OpenCode V2.`);
            ye(
...
${info.note}`
        ...
        `Instalación requerida (${platform})`
    );
...
async function runUninstallStackCommand(scope) {
    ...
    const confirm = await me({
        message:
            '¿Remover la configuración del stack (CodeGraph, Engram) del proyecto? Los binarios globales no se tocan.',
    });
    onCancel(confirm);
    if (!confirm) {
        fe('Cancelado.');
...
async function runDoctorCommand() {
    ...
    const pluginActive = pluginPaths.some((p) => existsSync9(p));
    try {
        if (!existsSync9(statePath)) {
            ...
            if (pluginActive) {
                console.log(
                    `✅ controller: plugin active (rev ${parsed.revision || 0} state ${parsed.state || 'unknown'})`
                );
            } else {
            ...
            if (parsed.degradedEditsCount > 0)
                console.log(
                    `⚠️ degraded: confirmation not audited in controller (degradedEditsCount=${parsed.degradedEditsCount})`
                );
            ...
            if (parsed.sensitiveAccess)
                console.log(
                    `ℹ️ sensitiveAccess: allowed=${parsed.sensitiveAccess.allowed || 0} denied=${parsed.sensitiveAccess.denied || 0} blocked=${parsed.sensitiveAccess.blockedAttempts || 0}`
                );
            ...
            if (
                (parsed.state === 'EXECUTING_INLINE' || parsed.state === 'EXECUTING_SUBAGENTS') &&
                parsed.expectedTasks
            ) {
                const pending = parsed.expectedTasks.filter(
                    (id) => !parsed.tasks?.[id] || parsed.tasks[id].status !== 'COMPLETED'
                ).length;
                const lastHandoffAge = parsed.lastHandoff ? Date.now() - parsed.lastHandoff.ts : Infinity;
                if (pending === 0 && lastHandoffAge > 60000) {
                    console.log(
                        '⚠️ EXECUTING_* con pending==0 y lastHandoff >60s sin progreso — sugerir implementation_complete manual'
                    );
                    hasWarn = true;
                }
            }
    ...
    try {
        ...
        if (existsSync9(lockPid) || existsSync9(lockTs)) {
            let ageStr = '';
            try {
                const ts = parseInt(readFileSync7(lockTs, 'utf-8'), 10);
                const age = Date.now() - ts;
                ageStr = `${Math.floor(age / 1000)}s`;
                const pid = readFileSync7(lockPid, 'utf-8').trim();
                let alive = false;
                try {
    ...
    for (const t of tools) {
        const p = join11(opencodeDir, 'tools', t, 'bin', t);
        const pExe = p + '.exe';
        check(
            `tool ${t}: ${existsSync9(p) || existsSync9(pExe) ? 'found' : 'missing'}`,
            existsSync9(p) || existsSync9(pExe),
            true
        );
    }
    try {
        const manifest = JSON.parse(readFileSync7(join11(cwd, 'manifest.json'), 'utf-8'));
    ...
    try {
        await Promise.resolve().then(() => init_doctor_v2());
    ...
    if (existsSync9(statePath)) {
        try {
            const s = JSON.parse(readFileSync7(statePath, 'utf-8'));
    ...
    try {
        ...
        if (!existsSync9(cacheDir)) {
            ...
            for (const f of files) {
                try {
                    total += statSync3(join11(cacheDir, f)).size;
                } catch {}
            }
            ...
            try {
                ...
                if (s.cacheHitCount !== undefined) {
                    console.log(
                        `ℹ️ cacheHitCount=${s.cacheHitCount} cacheMissCount=${s.cacheMissCount || 0} tokenSavingEstimate=${s.tokenSavingEstimate || 0}`
                    );
                }
            } catch {}
        }
    ...
    try {
        const coreSrc = join11(cwd, 'src', 'controller-core.ts');
        const coreMcp = join11(cwd, 'assets', 'mcp', 'ostacky-controller', 'controller-core.js');
        const corePlugin = join11(cwd, 'assets', 'plugins', 'controller-core.ts');
        const coreSync = existsSync9(coreSrc) && existsSync9(coreMcp) && existsSync9(corePlugin);
        if (coreSync) {
            const srcHash = readFileSync7(coreSrc, 'utf-8').slice(0, 100);
            const mcpHash = readFileSync7(coreMcp, 'utf-8').slice(0, 100);
            const ok =
                readFileSync7(coreMcp, 'utf-8').includes('STATES') &&
                readFileSync7(corePlugin, 'utf-8').includes('STATES');
            check(`controller-core: synced (${ok ? 'STATES present' : 'mismatch'})`, ok);
            if (!ok)
                console.log(
                    '  Run: bun run scripts/sync-controller-core.ts (if exists) or copy src/controller-core.ts'
                );
        } else {
            check('controller-core: synced', false, true);
        }
        const auditPath = join11(opencodeDir, 'ostacky-audit.jsonl');
        if (existsSync9(auditPath)) {
            const sz = statSync3(auditPath).size;
            const lines = readFileSync7(auditPath, 'utf-8')
                .split(
                    `
`
                )
        ...
        const ostackyMd = readFileSync7(join11(cwd, 'assets', 'agents', 'ostacky.md'), 'utf-8');
        ...
        const pluginPath = join11(cwd, 'assets', 'plugins', 'ostacky-plugin.ts');
        if (existsSync9(pluginPath)) {
            const plugin = readFileSync7(pluginPath, 'utf-8');
    ...
    try {
        const giPath = join11(cwd, '.gitignore');
        if (!existsSync9(giPath)) {
            console.log('⚠️ .gitignore: missing (run npx ostacky install --scope local to create)');
            hasWarn = true;
        } else {
            const gi = readFileSync7(giPath, 'utf-8');
            const needed = [
                '.opencode/tools/',
                '.opencode/cache/',
                '.opencode/ostacky-state.json',
    ...
    try {
        ...
        if (!existsSync9(secPath)) {
            console.log('⚠️ src/security.ts: missing (source-of-truth)');
            hasWarn = true;
        } else {
            const sec = readFileSync7(secPath, 'utf-8');
            ...
            if (hasSensitiveDefault && hasBashRe && hasIsSensitive && hasExtract) {
                ...
                console.log(
                    '⚠️ src/security.ts: missing exports (SENSITIVE_DEFAULT/BASH_SENSITIVE_RE/isSensitive/extractPathsFromBash)'
                );
                hasWarn = true;
            }
        }
...
async function runStatusCommand(args) {
    ...
    try {
        const parsed = JSON.parse(readFileSync7(statePath, 'utf-8'));
        const completed = Object.values(parsed.tasks || {}).filter((t) => t.status === 'COMPLETED').length;
        const expected =
            parsed.expectedTaskCount ?? parsed.expectedTasks?.length ?? Object.keys(parsed.tasks || {}).length;
        ...
        if (isJson) {
            ...
            console.log(
                `${parsed.state} rev ${parsed.revision}${degraded} tasks ${completed}/${expected}${lastHandoff}`
            );
            if (parsed.lastProposal)
                console.log(
                    `lastProposal: ${parsed.lastProposal.summary} shownToUser=${parsed.lastProposal.shownToUser}`
                );
        }
    } catch (e) {
