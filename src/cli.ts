...
async function runDoctorCommand() {
        ...
        if (!existsSync(statePath)) {
            ...
            if (
                ...
                const pending = parsed.expectedTasks.filter(
                    (id: string) => !parsed.tasks?.[id] || parsed.tasks[id].status !== 'COMPLETED'
                ).length;
    ...
    for (const t of tools) {
        ...
        const pExe = p + '.exe';
        check(
            `tool ${t}: ${existsSync(p) || existsSync(pExe) ? 'found' : 'missing'}`,
            existsSync(p) || existsSync(pExe),
            true
        );
    }
    ...
    try {
        ...
        if (coreSync) {
            ...
            const ok =
                readFileSync(coreMcp, 'utf-8').includes('STATES') &&
                readFileSync(corePlugin, 'utf-8').includes('STATES');
