  ...
  try {
    ...
    send({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "ostacky-installer", version: "0.6.1" },
      },
    });
  });
  } finally {
...
export async function installMcpServer(
  ...
  const statePath = item.name === "ostacky-controller"
    ? join(paths.root, "ostacky-state.json")
    : undefined;
  ...
  try {
    ...
    try {
      ...
      try { restoreFileSnapshot(configSnapshot); } catch {}
      try { restoreFileSnapshot(lockfileSnapshot); } catch {}
      try { promotion.rollback(); } catch {}
      throw error;
    }
  } finally {
