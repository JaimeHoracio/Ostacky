...
describe('B2: handoff persistence', () => {
    it('setHandoff persists and returns the handoff with timestamp', async () => {
        ...
        const result = await c.setHandoff({ summary: 'Implementing v0.9.2', nextSteps: ['a', 'b'] });
        expect(result.ok).toBe(true);
        expect(result.lastHandoff.summary).toBe('Implementing v0.9.2');
...
describe('B3: pruneStaleSkills', () => {
    it('removes skill directories not declared in the manifest', async () => {
        ...
        const manifest = {
            ...
            commands: [],
            mcpServers: [],
            skills: [{ name: 'brainstorming', version: '0.9.2' }],
        };
        const paths = {
            root: tmp,
            agents: '',
