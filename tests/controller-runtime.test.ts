...
describe("B2: handoff persistence", () => {
    it("setHandoff persists and returns the handoff with timestamp", async () => {
        ...
        const result = await c.setHandoff({ summary: "Implementing v0.7.0", nextSteps: ["a", "b"] });
        expect(result.ok).toBe(true);
        expect(result.lastHandoff.summary).toBe("Implementing v0.7.0");
...
describe("B1: consecutiveFailures and degraded mode", () => {
    ...
    it("enters degraded mode after 3 consecutive persist failures", async () => {
    ...
    }, { timeout: 30_000 }); // 3 fails × ~100ms with lockMaxAttempts=1
});
...
describe("B3: pruneStaleSkills", () => {
    it("removes skill directories not declared in the manifest", async () => {
        ...
        const manifest = {
            ...
            commands: [],
            mcpServers: [],
            skills: [{ name: "brainstorming", version: "0.7.0" }],
        };
        const paths = {
            root: tmp,
            agents: "",
